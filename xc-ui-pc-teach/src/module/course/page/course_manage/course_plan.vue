<template>
  <div>
    <el-button type="primary" @click="teachplayFormVisible = true;teachplanId='null'">添加课程计划</el-button>
    <el-tree
      :data="teachplanList"
      :props="defaultProps"
      node-key="id"
    ...
    <el-dialog title="添加课程计划" :visible.sync="teachplayFormVisible">
      ...
      <el-form ref="teachplanActive" :model="teachplanActive" label-width="140px" style="width:600px;"
               :rules="teachplanRules">
        <el-form-item label="上级结点" prop="parentid">
        ...
        <el-form-item label="课程类型" prop="ptype">
        ...
        <el-form-item label="学习时长（分钟）  请输入数字" prop="timelength">
...
//        alert(this.teachplanId)
      ...
      addTeachplan() {
       //alert(this.teachplanId);
        //校验表单
        this.$refs.teachplanActive.validate((valid) => {
          if (valid) {
            //调用api方法
            //将课程id设置到teachplanActive
            this.teachplanActive.courseid = this.courseid
            console.log(this.teachplanId)
            courseApi.addTeachplan(this.teachplanId, this.teachplanActive).then(res => {
              if (res.success) {
      ...
      edit(data) {
        this.teachplanId = data.id
       // alert(this.teachplanId)
        this.findTeachplan1(this.teachplanId);
        this.teachplayFormVisible = true
      },
      remove(node, data) {
        console.log(data.id);
        this.$confirm('确认级联删除该课程计划(包括与媒资视频的关联)吗?', '提示', {}).then(() => {
          ...
          <span
            ...
            <span>
              <el-button style="font-size: 12px;" type="text"
                         on-click={() => this.choosevideo(data)}><label style="color:red">{data.mediaFileOriginalName}</label>&emsp;&emsp;&emsp;选择视频</el-button>
      ...
      findTeachplan() {
        this.mediaFormVisible = false;//关闭窗口
        this.teachplanList = []
        //查询课程计划
      ...
      findTeachplan1(id) {
        //查询课程计划
        courseApi.findTeachplan(id).then(res => {
          console.log(res);
          if (res) {
