        ...
        <el-card class="box-card">
          ...
          <div class="text item">
            <div v-if="course.status == '202001'">
              状态：制作中<br/>
              <el-button type="primary"  @click.native="rejectMedia" >新课程发布</el-button>
            </div>
            <div v-else-if="course.status == '202003'">
              状态：已下线
              <br/><br/>
              <span><a :href="'http://www.yok.com/course/detail/'+this.courseid+'.html'" target="_blank">点我查看课程详情页面 </a> </span>
            </div>
            <div v-else-if="course.status == '202002'">
              状态：已发布<br/>
              <el-button type="primary"  @click.native="rejectMedia" >修改发布</el-button>
              <br/><br/>
              <span><a :href="'http://www.yok.com/course/detail/'+this.courseid+'.html'" target="_blank">点我查看课程详情页面 </a> </span>
            </div>
          </div>
...
export default{
  ...
  methods:{
    ...
    rejectMedia(){
      courseApi.reject(this.courseid).then(res=> {
      console.log(res)
        if (res.success){
          this.publish()
        }else{
          var msg = res.message
          this.$confirm(msg+"点击确定将继续上传", '提示', {}).then(() => {
            this.publish()
          })
        }
      })
